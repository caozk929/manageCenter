...
angular.module("w5c.validator", ["ng"])
    .provider('w5cValidator', [function () {
        var defaultRules = {
                ...
                min           : "该选项输入值不能小于{min}",
                mobile        : "手机号码格式不正确",
                tel           : "固定电话格式不正确(区号-座机号)",
                customizer    : "自定义验证不通过"

            },
            elemTypes = ['mobile','tel','text', 'password', 'email', 'number', 'url', 'tel', 'hidden', ['textarea'], ['select'], ['select-multiple'], ['select-one'], 'radio', 'checkbox'];
        ...
        var validatorFn = function () {
            ...
            this.defaultShowError = function (elem, errorMessages) {
                ...
                if(!$inputgroup || $inputgroup.length==0){
                    ...
                    if($elem.next(".error-con").length <= 0){
                        // $elem.after('<span class="w5c-error">' + errorMessages[errorMessages.length-1] + '</span>');
                        $elem.after('<div class="error-con"><span class="error-tip" style="">' + errorMessages[errorMessages.length-1] + '</span></div>');
                    }
                }else{
                    ...
                    if($inputgroup.find(".error-con").length <= 0){
                        // $elem.after('<span class="w5c-error">' + errorMessages[errorMessages.length-1] + '</span>');
                        $inputgroup.append('<div class="error-con"><span class="error-tip" style="">' + errorMessages[errorMessages.length-1] + '</span></div>');
                    }
                }
            };
            this.defaultShowTip = function (elem, msg) {
                var $elem = angular.element(elem),
                    $group = getParentGroup($elem); 
            ...
            this.defaultRemoveError = function (elem) {
                ...
                var $next = $elem.next();
                if ($next.hasClass && $next.hasClass("error-con")) {
                    $next.remove();
                }
        ...
        validatorFn.prototype = {
            ...
            showTip       : function (elem,ctrl) {
                var tip = $(elem).attr('tip');
                if(!tip){
                    tip = $(elem).attr('placeholder');
                    if(!tip)
                    return;
                }
                angular.element(elem).addClass("valid").removeClass("error");
                getParentInputGroup(elem).find('.error-con').remove();
                return this.defaultShowTip(elem, tip);
            },
...
(function () {
    angular.module("w5c.validator")
            ...
            return {
                ...
                controller: ['$scope', function ($scope) {
                    ...
                    this.removeElementValidation = function (name) { 
                        console.log("this.validElements:"+angular.toJson(this.validElements));
                        var index = -1;
                    ...
                    this.initElement = function (elem) {
                        ...
                        if (w5cValidator.elemTypes.toString().indexOf(elem.type) > -1 && !w5cValidator.isEmpty(elem.name) && !/^\d/.test(elem.name)) {
                            ...
                            var idx = -1;
                            for(i=0;i<this.validElements.length;i++){
                                if(this.validElements[i] == elem.name){
                                    idx = i;
                                }
                            }
                            if (idx < 0) {
                            ...
                            $elem.bind("focus", function () {
                                    ...
                                    $timeout(function () {
                                        w5cValidator.removeError($elem, ctrl.options);
                                        w5cValidator.showTip($elem,ctrl);
                                    }, 50);
                                });
                        }
                    };
                ...
                link      : function (scope, form, attr, ctrls) {
                    ...
                    var doValidate = function () {
                        ...
                        for (var i = 0; i < formElem.length; i++) {
                            ...
                            var idx2 = -1;
                            for(k=0;k<ctrl.validElements.length;k++){
                                if(ctrl.validElements[k] == elemName){
                                    idx2 = k;
                                }
                            }
                            if (elemName && idx2 >= 0) {
        ...
        .directive("w5cMobile", ['$timeout', function ($timeout) {
            'use strict';
            return {
                require: ["^form", "ngModel"],
                link   : function (scope, elem, attrs, ctrls) {
                    ...
                    var $validate = function () {
                        ...
                        //ngModelCtrl.$setValidity("mobile", (/^1[3|5|4|7|8|2|1][0-9]\d{4,8}$/).test(newValue)); 
                        ngModelCtrl.$setValidity("mobile", (/^(0|86|17951)?(13[0-9]|15[0-9]|17[0-9]|18[0-9]|14[0-9])[0-9]{8}$/).test(newValue)); 
                    };
                    // var associate = ctrls[0][attrs.associate];
                    // associate && associate.$viewChangeListeners.push($validate);
                    ngModelCtrl.$viewChangeListeners.push($validate);
        ...
        .directive("w5cEmail", ['$timeout', function ($timeout) {
            'use strict';
            return {
                require: ["^form", "ngModel"],
                link   : function (scope, elem, attrs, ctrls) {
                    ...
                    var $validate = function () { 
                        var newValue = ngModelCtrl.$viewValue; 
                        ngModelCtrl.$setValidity("email", (/^(\w)+(\.\w+)*@(\w)+((\.\w+)+)$/).test(newValue)); 
                    }; 
                    ngModelCtrl.$viewChangeListeners.push($validate);
                }
            };
        }])
        .directive("w5cTel", ['$timeout', function ($timeout) {
            'use strict';
            return {
                require: ["^form", "ngModel"],
                link   : function (scope, elem, attrs, ctrls) {
                    ...
                    var $validate = function () { 
                        var newValue = ngModelCtrl.$viewValue; 
                        ngModelCtrl.$setValidity("tel", (/^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})?$/).test(newValue)); 
                    }; 
                    ngModelCtrl.$viewChangeListeners.push($validate);
                }
            };
        }])
        .directive("w5cTel2", ['$timeout', function ($timeout) {
            'use strict';
            return {
                require: ["^form", "ngModel"],
                link   : function (scope, elem, attrs, ctrls) {
                    ...
                    var $validate = function () { 
                        var newValue = ngModelCtrl.$viewValue; 
                        ngModelCtrl.$setValidity("tel", (/^(0[0-9]{2,3}\-)?([2-9][0-9]{6,7})+(-[0-9]{1,4})?$/).test(newValue)); 
                    }; 
                    ngModelCtrl.$viewChangeListeners.push($validate);
                }
            };
